...
module.exports = class ChromeDevWebpackPlugin {
  ...
  constructor(options) {
    ...
    this.manifestSync = new ManifestSync(null, this.pathToPackageJson);
      ...
      if(requireMap && requireMap.length) {
        ...
        if (manifestJson.background.scripts && 0 < manifestJson.background.scripts.length) {
          ...
          manifestJson.background.scripts.forEach( (file) => {
            if ("undefined" !== typeof mappedValues[file]) {
  ...
  initialize (compilation) {
    ...
    //Setup the sync system in case it wasn't setup.
    if ("object" !== typeof this.manifestSync || !this.manifestSync) {
      this.manifestSync = new ManifestSync();
    }
    return this.findManifestSource(compilation).then ( () => {
  ...
  findManifestSource (compilation) {
    ...
    return new mPromise( (resolve, reject) => {
      ...
      this.log("this.manifestSync.manifest:", this.manifestSync.manifest);
      if ("object" !== typeof this.manifestSync.manifest) {
        ...
        if(this.manifestInput) {
          ...
          return resolve(this.manifestSync.manifest);
        } else if (this.manifestOutput
          ...
          fs.exists(pathToContextManifest, (exists) => {
            this.mapFilesRelativeToManifest = exists;
            resolve(this.manifestSync.manifest);
          });
          return;
        } else if(this.context) {
          if (fs.existsSync(pathToContextManifest)) {
            this.log("Will use the manifest.json found in the context path.");
            this.manifestSync.setManifest(pathToContextManifest);
