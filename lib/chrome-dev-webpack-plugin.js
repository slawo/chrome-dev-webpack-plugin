...
var BundleFilesMapper = require("./bundle-files-mapper");
...
module.exports = class ChromeDevWebpackPlugin {
  ...
  constructor(options) {
    ...
    if(!options.hasOwnProperty("map") || options.map) {
      this.bundleFilesMapper = new BundleFilesMapper();
    }
  ...
  apply (compiler) {
    ...
    if (this.bundleFilesMapper) {
      this.bundleFilesMapper.context = this.context;
    }
  ...
  runPluginRound (compilation) {
    ...
    return this.manifestSync.sync().then( (manifestJson) => {
      ...
      return this.runFiles(compilation, manifestJson);
    }).then ( (manifestJson) => {
      if (manifestJson) {
  ...
  runFiles(compilation, manifestJson) {
    return new mPromise( (resolve) => {
      if (this.bundleFilesMapper) {
        resolve(this.bundleFilesMapper.run (compilation, manifestJson));
      }
      else {
